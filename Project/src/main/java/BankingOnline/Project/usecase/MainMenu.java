package BankingOnline.Project.usecase;

import java.util.Scanner;

import BankingOnline.Project.service.AccountsService;
import BankingOnline.Project.serviceImpl.AccountsServiceImpl;
import BankingOnline.Project.service.AccountManagerService;
import BankingOnline.Project.serviceImpl.AccountManagerServiceImpl;
import BankingOnline.Project.service.UserService;
import BankingOnline.Project.serviceImpl.UserServiceImpl;


public class MainMenu {
    public static void main(String[] args) {
    	AccountsService accounts = new AccountsServiceImpl();
    	UserService user = new UserServiceImpl();
    	AccountManagerService accountManager = new AccountManagerServiceImpl();
    	
    	
    	 String email;
         long account_number;
         int choice1;

         Scanner scanner = new Scanner(System.in);
        do{
             System.out.println("*** WELCOME TO BANKING SYSTEM ***");
             System.out.println();
             System.out.println("1. Register");
             System.out.println("2. Login");
             System.out.println("3. Exit");
             System.out.println("Enter your choice: ");
             choice1 = scanner.nextInt();
             switch (choice1){
                 case 1:
                
                     user.register();
                     break;
                 case 2:
                     email = user.login();
                     if(email!=null){
                         System.out.println();
                         System.out.println("User Logged In!");
                         if(!accounts.account_exist(email)){
                             System.out.println();
                             System.out.println("1. Open a new Bank Account");
                             System.out.println("2. Exit");
                             if(scanner.nextInt() == 1) {
                                 account_number = accounts.open_account(email);
                                 System.out.println("Account Created Successfully");
                                 System.out.println("Your Account Number is: " + account_number);
                             }else{
                                 break;
                             }

                         }
                         account_number = accounts.getAccount_number(email);
                         int choice2 = 0;
                         while (choice2 != 5) {
                             System.out.println();
                             System.out.println("1. Debit Money");
                             System.out.println("2. Credit Money");
                             System.out.println("3. Transfer Money");
                             System.out.println("4. Check Balance");
                             System.out.println("5. Log Out");
                             System.out.println("Enter your choice: ");
                             choice2 = scanner.nextInt();
                             
                             switch (choice2) {
                                 case 1:
                                	 System.out.print("Enter Amount you want to withdrawal: ");
                                	 double withAmount = scanner.nextDouble();
                                     accountManager.debit_money(account_number,withAmount);
                                     break;
                                 case 2:
                                	 System.out.print("Enter Amount you want to deposit: ");
                                	 double depoAmount = scanner.nextDouble();
                                     accountManager.credit_money(account_number,depoAmount);
                                     break;
                                 case 3:
                                	 System.out.print("Enter account number you want to transfer: ");
                                	long toAccount = scanner.nextLong();
                                	 System.out.print("Enter Amount you want to transfer: ");
                                	 double transAmount = scanner.nextDouble();
                                     accountManager.transfer_money(toAccount,transAmount);
                                     break;
                                 case 4:
                                     accountManager.getBalance(account_number);
                                     break;
                                 case 5:
                                     break;
                                 default:
                                     System.out.println("Enter Valid Choice!");
                                     break;
                             }
                         }

                     }
                     else{
                         System.out.println("Incorrect Email or Password!");
                     }
                 case 3:
                     System.out.println("THANK YOU FOR USING BANKING SYSTEM!!!");
                     System.out.println("Exiting System!");
                     return;
                 default:
                     System.out.println("Enter Valid Choice");
                     break;
             }

    } while(choice1!=3);

}
}
